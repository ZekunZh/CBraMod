...
class LoadDataset(object):
    ...
    def load_path(self):
        ...
        for subject_num in range(1, 101):
            subject_dirs_seq.append(
                os.path.join(self.seqs_dir, f"ISRUC-group1-{subject_num}")
            )
            subject_dirs_labels.append(
                os.path.join(self.labels_dir, f"ISRUC-group1-{subject_num}")
            )
        ...
        for subject_seq, subject_label in zip(subject_dirs_seq, subject_dirs_labels):
            ...
            for seq_fname, label_fname in zip(seq_fnames, label_fnames):
                subject_pairs.append(
                    (
                        os.path.join(subject_seq, seq_fname),
